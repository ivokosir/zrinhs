module Error (Error (..), printErrors) where
...
data Error = Error SourcePos SourcePos String | ParseError ParseError
...
formatError source (Error start end message) =
  getLines start end source ++ "\n" ++ message
formatError source (ParseError error) =
